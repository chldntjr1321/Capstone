...
# API 요청 URL - HTTP로 변경
url = 'http://apis.data.go.kr/B500001/rwis/waterPressure_1/list_1'
params = {
...
try:
    ...
    filtered_items = [item for item in items if str(item.get('dataItemTagsn')) == '7339']
    ...
    for item in filtered_items:
        output_data.append({
            ...
            '데이터 값': item.get('dataVal', '없음'),
            '단위': item.get('itemUnit', '없음')
        })

    # JSON 파일로 저장
    with open('water_pressure_data.json', 'w', encoding='utf-8') as json_file:
